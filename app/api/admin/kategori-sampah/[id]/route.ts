...
export async function GET(
  ...
  try {
    const result = await apiGet<KategoriSampahItem>(
      `/api/v1/kategori-sampah/${id}`,
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const backendRes = await fetch(
        `${BASE_URL}/api/v1/kategori-sampah/${id}`,
        {
          method: "PUT",
      ...
      return NextResponse.json({
        success: true,
        message: json.message || "Kategori sampah berhasil diperbarui",
        data: json.data,
      });
    }
    ...
    const status = (error as { status?: number }).status || 500;
    const message =
      error instanceof Error ? error.message : "Gagal memperbarui kategori sampah";
    return NextResponse.json(
      { success: false, message, data: null },
      { status },
    );
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({
      success: true,
      message: result.message || "Kategori sampah berhasil dihapus",
      data: null,
    });
  } catch (error: unknown) {
    const status = (error as { status?: number }).status || 500;
    const message =
      error instanceof Error ? error.message : "Gagal menghapus kategori sampah";
    return NextResponse.json(
      { success: false, message, data: null },
      { status },
    );
