...
interface HadiahDeleteDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
  hadiah: HadiahItem | null;
}

export default function HadiahDeleteDialog({
  open,
  onClose,
  onSuccess,
  hadiah,
}: HadiahDeleteDialogProps) {
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/hadiah/${hadiah.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (!res.ok || !json.success) {
        setError(json.message || "Gagal menghapus hadiah.");
        setLoading(false);
        return;
      }
    ...
    <Dialog open={open} onClose={onClose} className="max-w-md mx-4">
      <div className="p-6">
        ...
        <p className="text-sm text-zinc-600 mb-4">
          Hadiah <span className="font-semibold">{hadiah?.namaHadiah}</span> akan dihapus permanen. Data ini tidak dapat dikembalikan, lanjutkan?
        </p>
