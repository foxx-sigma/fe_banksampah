...
interface KategoriDeleteDialogProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
  kategori: KategoriSampahItem | null;
}
...
export default function KategoriDeleteDialog({
  ...
  async function handleDelete() {
    ...
    try {
      const res = await fetch(`/api/admin/kategori-sampah/${kategori.id}`, {
        method: "DELETE",
        credentials: "include",
      });
      ...
      if (!res.ok || !json.success) {
        setError(json.message || "Gagal menghapus kategori sampah.");
        setLoading(false);
        return;
      }
    ...
    <Dialog open={open} onClose={onClose} className="max-w-md mx-4">
      <div className="p-6">
        ...
        <p className="text-sm text-zinc-600 mb-4">
          Kategori <span className="font-semibold">{kategori?.namaKategori}</span> akan dihapus permanen. Data ini tidak dapat dikembalikan, lanjutkan?
        </p>
